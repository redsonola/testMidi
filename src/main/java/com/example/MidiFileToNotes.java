 ...
 */
 ...
 public class MidiFileToNotes {
	 ...
	 void processPitchesAsTokens() {
		 ...
		 // extract all the pitches and notes from the melody & put them in MIDI style-note ordering using an insertion-style sort
		 for (int i = 0; i < phrases.length; i++) {
			 jm.music.data.Note[] notes = phrases[i].getNoteArray();
			 for (int j = 0; j < notes.length; j++) {
 
				 int placeToAdd = rhythms.size(); //default, add at end
				 for( int k=0; k<startTimes.size(); k++ )
				 {
					 if( startTimes.get(k) == phrases[i].getNoteStartTime(j)  )
					 {
						 rhythms.set(k, 0.0); 
						 placeToAdd = k+1; 
					 }
				 ...
				 rhythms.add(placeToAdd, notes[j].getRhythmValue() );
